...
# omitted because the grammar oracle's input-driven derivation doesn't yet
# cover those statement classes (see DriftReason::RecognizerHole entries in
# drift.rs). plpgsql / replication / optimizer torture tests are also out of
# scope — they exercise features DSQL doesn't support and don't surface
# lint gaps.
FILES=(
    create_table.sql
    create_index.sql
