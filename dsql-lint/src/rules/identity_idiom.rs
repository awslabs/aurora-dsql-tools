...
//!   SERIAL-idiom collapse). The trailing `SELECT setval('t_id_seq', ...)`
//!   pg_dump emits is **left untouched**: an inline identity column creates an
//!   implicit sequence of the same name, so the setval lands on it and the
//!   counter is advanced — no manual reset needed (unlike the SERIAL path).
...
/// matches `table` and declares `column`, or `None`.
