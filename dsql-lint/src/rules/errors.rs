use sqlparser::ast::{
    ...
    CopyTarget, CreateTableOptions, DataType, DeclareType, Expr, GeneratedAs, SequenceOptions, Set,
    SqlOption, Statement, TableConstraint, TransactionIsolationLevel, TransactionMode,
        ...
        if matches!(
            ...
            if in_alter_table {
                ...
                diagnostics.push(error(
                    ...
                    FixResult::Fixed(format!("Replaced {} with BIGINT GENERATED BY DEFAULT AS IDENTITY (CACHE 1) on column `{}`", type_str, col.name)),
                ));
            }
        }
    }
...
fn check_create_index(stmt: &mut Statement, raw_sql: &str, diagnostics: &mut Vec<Diagnostic>) {
    ...
    if let Some(index_type) = ci.using.take() {
