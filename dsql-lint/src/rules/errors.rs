...
/// `GENERATED <generated_as> AS IDENTITY (CACHE <cache>)`. The identity-collapse
/// rule passes the source cache through so a tuned `CACHE 65536` isn't reset to
/// 1; out-of-range values are clamped downstream by `validate_cache_options`.
pub(crate) fn identity_with_cache(generated_as: GeneratedAs, cache: i64) -> ColumnOptionDef {
    ColumnOptionDef {
        name: None,
        option: ColumnOption::Generated {
            generated_as,
            sequence_options: Some(vec![cache_option(cache)]),
...
/// `GENERATED <generated_as> AS IDENTITY (CACHE 1)` — the safe-default form.
pub(crate) fn identity_cache_1(generated_as: GeneratedAs) -> ColumnOptionDef {
    identity_with_cache(generated_as, 1)
}
...
pub(crate) fn identity_by_default_cache_1() -> ColumnOptionDef {
    identity_cache_1(GeneratedAs::ByDefault)
}
