...
/// Construct a `GENERATED <generated_as> AS IDENTITY (CACHE 1)` column option.
/// Used by the DSQL-native identity-collapse rule, which must preserve the
/// source's `ALWAYS` / `BY DEFAULT` choice rather than forcing one.
