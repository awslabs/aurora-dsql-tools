...
//! ... UNIQUE (...)` statements into inline UNIQUE constraints on the
//! preceding `CREATE TABLE`.
//!
//! ## Why collapse?
//! PostgreSQL stores UNIQUE constraints separately from the table
...
//! ```
//!
//! DSQL accepts UNIQUE only when defined inline on the `CREATE TABLE`
//! itself; standalone `ALTER TABLE ... ADD CONSTRAINT ... UNIQUE` is
//! rejected. The DSQL-idiomatic shape is:
//!
//! ```sql
//! CREATE TABLE public.users (
//!     id integer NOT NULL,
...
//! ```
//!
//! This module folds the ALTER's `UniqueConstraint` straight onto the
//! CREATE TABLE's `constraints` vector — no field reshuffling, no
...
//!   `AddConstraint { constraint: TableConstraint::Unique(...) }`. A
//!   multi-op ALTER (e.g. `ADD CONSTRAINT a UNIQUE (...), ADD CONSTRAINT
//!   b UNIQUE (...)`) is left to the per-statement rule, which still
//!   surfaces it as Unfixable.
...
//!   it.)
//! - The constraint variant is plain `Unique(_)`. The PG-specific
//!   `UniqueUsingIndex` variant (created by `ALTER TABLE ... ADD
//!   CONSTRAINT ... UNIQUE USING INDEX ...`) is skipped — it carries
//!   different semantics and DSQL has its own rule for it.
//!
...
#[derive(Debug, Clone)]
pub(crate) struct UniqueAddIdiom {
    /// Normalized ref of the table being altered.
    pub table: NameRef,
    /// The full UNIQUE constraint to fold onto the CREATE TABLE.
...
/// ... UNIQUE (...)` in `stmts`. Each result names a CREATE TABLE that
/// is already present in `stmts` (otherwise the fold has no target —
/// the per-statement rule handles that case as Unfixable).
pub(crate) fn detect_alter_add_unique(stmts: &[Statement]) -> Vec<UniqueAddIdiom> {
    ...
    for (alter_index, stmt) in stmts.iter().enumerate() {
        ...
        let TableConstraint::Unique(uniq) = constraint else {
            // `UniqueUsingIndex` is a different shape; skip it.
            continue;
        };
        ...
        idioms.push(UniqueAddIdiom {
            table: table_ref,
            constraint: uniq.clone(),
...
pub(crate) fn check_alter_add_unique(parts: &[(usize, String)], diagnostics: &mut Vec<Diagnostic>) {
    ...
    for idiom in detect_alter_add_unique(&parsed) {
        ...
        diagnostics.push(Diagnostic {
            ...
            message: format!(
                "ALTER TABLE ADD CONSTRAINT ... UNIQUE on `{table}` is not supported in DSQL. \
                 Define UNIQUE inline on the CREATE TABLE.",
                table = idiom.table.1,
            ),
            suggestion: format!(
                "Move the UNIQUE constraint into the `CREATE TABLE {table}` definition.",
                table = idiom.table.1,
            ),
            fix_result: FixResult::Fixed(format!(
                "Folded `{}` UNIQUE constraint into the CREATE TABLE definition.",
                idiom.table.1,
            )),
        });
    }
...
/// Fix-mode pass: rewrite each idiom's CREATE TABLE to embed the UNIQUE
/// constraint inline, then drop the redundant ALTER TABLE part. Runs
/// BEFORE the per-statement loop so the per-statement `AtUnsupportedAddUnique`
/// rule never sees these statements (the parts are gone by then).
pub(crate) fn fix_alter_add_unique(
    ...
    let (parsed, parsed_to_part) = parse_parts(parts);
    let idioms = detect_alter_add_unique(&parsed);
    if idioms.is_empty() {
        return;
    }
    ...
    for idiom in &idioms {
        ...
        for stmt in &mut stmts {
            if let Statement::CreateTable(ct) = stmt {
                if normalize_object_name(&ct.name).is_some_and(|n| refs_match(&n, &idiom.table)) {
                    ct.constraints
                        .push(TableConstraint::Unique(idiom.constraint.clone()));
                    folded = true;
                    break;
                }
            }
        ...
        diagnostics.push(Diagnostic {
            ...
            suggestion: format!(
                "Moved the UNIQUE constraint into the `CREATE TABLE {table}` definition.",
                table = idiom.table.1,
            ),
            fix_result: FixResult::Fixed(format!(
                "Folded `{}` UNIQUE constraint into the CREATE TABLE definition.",
...
mod tests {
    ...
    #[test]
    fn detects_simple_add_unique_after_create_table() {
        let stmts = parse_ok(&[
    ...
    fn alter_before_create_table_is_not_a_match() {
        ...
        let stmts = parse_ok(&[
            "ALTER TABLE ONLY public.users ADD CONSTRAINT u UNIQUE (email)",
            "CREATE TABLE public.users (id integer NOT NULL, email text NOT NULL)",
        ]);
        let idioms = detect_alter_add_unique(&stmts);
        assert!(
    ...
    fn unrelated_table_is_not_folded_into_wrong_create() {
        ...
        let stmts = parse_ok(&[
            ...
            "ALTER TABLE ONLY public.orders ADD CONSTRAINT u UNIQUE (oid)",
        ]);
        let idioms = detect_alter_add_unique(&stmts);
    ...
    fn cross_file_alter_without_create_is_skipped() {
        ...
        // Unfixable rule.
        let stmts = parse_ok(&["ALTER TABLE ONLY public.users ADD CONSTRAINT u UNIQUE (email)"]);
        let idioms = detect_alter_add_unique(&stmts);
    ...
    fn unqualified_alter_matches_qualified_create() {
        ...
        let stmts = parse_ok(&[
            "CREATE TABLE public.users (id integer NOT NULL)",
            "ALTER TABLE ONLY users ADD CONSTRAINT u UNIQUE (id)",
        ]);
        let idioms = detect_alter_add_unique(&stmts);
    ...
    fn case_folded_unquoted_names_match() {
        ...
        let stmts = parse_ok(&[
            ...
            "ALTER TABLE ONLY MY_TABLE ADD CONSTRAINT u UNIQUE (id)",
        ]);
        let idioms = detect_alter_add_unique(&stmts);
        assert_eq!(idioms.len(), 1);
    }
    ...
    fn multi_op_alter_is_left_alone() {
        ...
        let stmts = parse_ok(&[
            ...
            "ALTER TABLE ONLY public.t \
                ADD CONSTRAINT u1 UNIQUE (a), \
                ADD CONSTRAINT u2 UNIQUE (b)",
        ]);
        let idioms = detect_alter_add_unique(&stmts);
        assert!(
            idioms.is_empty(),
