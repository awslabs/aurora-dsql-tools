...
use sqlparser::ast::{
    AlterTableOperation, Ident, ObjectName, PrimaryKeyConstraint, Statement, TableConstraint,
};
...
/// `unique_collapse` rather than sharing it: keeping each whole-list
/// rule's helper local prevents one rule's normalization choices from
/// quietly changing another rule's semantics.
...
/// `unique_collapse`. Without this preference an `ALTER TABLE ONLY public.t`
...
/// helper in `unique_collapse`; kept local to insulate this rule from
