...
use sqlparser::ast::{
    ...
    FunctionArg, FunctionArgExpr, FunctionArguments, GeneratedAs, ObjectName, Statement, Value,
    ValueWithSpan,
};
use sqlparser::dialect::PostgreSqlDialect;
use sqlparser::parser::Parser;

use super::errors::cache_1_option;
...
pub(crate) fn fix_serial_idioms(
    ...
    // Parse each part; remember which `parts` index each parsed statement came
    // from so detected idiom indices (into the parsed slice) map back to parts.
    let mut parsed: Vec<Statement> = Vec::new();
    let mut parsed_to_part: Vec<usize> = Vec::new();
    for (part_idx, (_, text)) in parts.iter().enumerate() {
        if let Ok(stmts) = Parser::parse_sql(&dialect, text.trim()) {
...
/// (CACHE 1)`, preserving its other options (e.g. `NOT NULL`). Mirrors the
/// inline-`SERIAL` mutation in `errors.rs` so both paths emit the same shape.
...
/// that belongs to a collapsed idiom. Matches case-insensitively and tolerates a
/// leading/trailing whitespace and an optional trailing `;`.
fn is_alter_sequence_owned_by(text: &str, sequence: &NameRef) -> bool {
    let trimmed = text.trim().trim_end_matches(';').trim();
    let lower = trimmed.to_ascii_lowercase();
    if !lower.starts_with("alter sequence ") || !lower.contains(" owned by ") {
        return false;
    }
    // The sequence name appears between `ALTER SEQUENCE` and `OWNED BY`.
    let after_kw = &trimmed["alter sequence ".len()..];
    let Some(owned_pos) = after_kw.to_ascii_lowercase().find(" owned by ") else {
        return false;
    };
    let name_part = after_kw[..owned_pos]
        .trim()
        .trim_end_matches("if exists")
        .trim();
    let name_ref = normalize_dotted_identifier(name_part);
