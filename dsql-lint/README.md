...
`--fix` also handles the `pg_dump` "SERIAL idiom": real PostgreSQL dumps
expand `SERIAL` into a `CREATE TABLE` + `CREATE SEQUENCE` + `ALTER SEQUENCE
OWNED BY` + `ALTER COLUMN SET DEFAULT nextval(...)` quartet. dsql-lint
collapses that pattern into a single `BIGINT GENERATED BY DEFAULT AS
IDENTITY (CACHE 1)` column, removing the redundant statements, so a
`pg_dump | dsql-lint --fix - | psql` pipeline works end-to-end.
