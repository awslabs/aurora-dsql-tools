...
`--dialect mysql` requires `--fix` — MySQL DDL must be translated before it can be applied, so there is no lint-only MySQL mode.
...
- **Types** — `tinyint(1)` → `BOOLEAN`; `tinyint`/`mediumint` → `SMALLINT`/`INTEGER`; unsigned integers widen to the next signed type (`bigint unsigned` → `NUMERIC`); `datetime` → `TIMESTAMP`; `year` → `INTEGER`; `enum(...)` → `VARCHAR(255)`; `set(...)` → `TEXT`; signed integer display widths (`int(11)`) are dropped.
