...
migration.sql:3: WARNING — Added NOT VALID to the foreign key. The constraint applies to new writes immediately, but existing rows are not validated until ALTER TABLE ASYNC ... VALIDATE CONSTRAINT completes
  → Validate existing rows with `ALTER TABLE ASYNC ... VALIDATE CONSTRAINT`, capture the returned `job_id`, and wait with `CALL sys.wait_for_job(job_id)`.
migration.sql:5: ERROR (unfixable) — ADD COLUMN 'status' with inline DEFAULT or NOT NULL constraint is not supported in DSQL.
  → Split into steps: CREATE TABLE with the column, or ADD COLUMN without constraints then backfill.
Fixed output written to: migration-fixed.sql
```

- `FIXED` — mechanical transformation that is equivalent on DSQL
- `WARNING` — fix applied but behavior changes; review required (e.g. async index not ready on return, weakened isolation level, widened SERIAL/sequence type)
...
`SET DEFAULT`, plus `MATCH SIMPLE`, `MATCH FULL`, and deferrable foreign keys.
`ALTER TABLE ... ADD CONSTRAINT ... FOREIGN KEY` must include `NOT VALID`.
`dsql-lint --fix` adds it, but validation of existing rows remains a separate
asynchronous DDL operation. Capture the validation statement's returned
`job_id` and wait with `CALL sys.wait_for_job(job_id)` before relying on the
constraint being valid. Cascading actions count toward Aurora DSQL's
...
Exit code 3 is the common case when `--fix` converts SERIAL to IDENTITY or rewrites synchronous index creation — the migration is DSQL-compatible but may require application-side changes. Scripts using `set -e` should handle it explicitly:
