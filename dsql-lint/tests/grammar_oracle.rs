...
//!
//! Loads `dsql_grammar.json` (see `grammar_oracle/grammar.rs`) and
//! asserts dsql-lint's verdicts
    ...
    if !unexpected_dsql_lint.is_empty() {
        ...
        msg.push_str(
            "\nFix by one of:\n  \
             ...
             - Extend the oracle (DriftReason::RecognizerHole)\n  \
...
/// Prints the full Postgres-corpus drift report. Ignored by default —
/// the agreement test already prints a count summary. Run with
/// `cargo test -p dsql-lint --test grammar_oracle pg_corpus_drift_report
/// -- --ignored --nocapture` to see the burndown surface.
#[test]
