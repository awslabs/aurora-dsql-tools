//! Vendored types matching the snowglobe-grammar JSON shape, plus an
...
pub struct Production {
    ...
    /// Snowglobe encodes lists like `expr_list = a_expr [, a_expr]*` this
    /// way: a single-choice production `[[a_expr]]` with `repetition: ","`.
...
impl Tok {
    fn matches_keyword(&self, kw: &str) -> bool {
        match self {
            ...
            Tok::Punct(c) => kw.len() == 1 && kw.chars().next() == Some(*c),
            _ => false,
        }
    }
