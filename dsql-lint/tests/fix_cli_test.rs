...
#[test]
fn fix_dialect_mysql_translates_mysqldump_ddl() {
    let dir = tempfile::tempdir().unwrap();
    let input = dir.path().join("dump.sql");
    std::fs::write(
        &input,
        "CREATE TABLE `users` (`id` int NOT NULL AUTO_INCREMENT, \
         `flag` tinyint(1), PRIMARY KEY (`id`)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;",
    )
    .unwrap();
    ...
    let output = dsql_lint_bin()
        ...
        .arg("mysql")
        .arg("-o")
        .arg(dir.path().join("out.sql").to_str().unwrap())
        .arg(input.to_str().unwrap())
        .output()
        .unwrap();

    let content = std::fs::read_to_string(dir.path().join("out.sql")).unwrap();
    assert!(!content.contains('`'), "backticks must be gone: {content}");
    assert!(
        content
            .to_uppercase()
            .contains("GENERATED BY DEFAULT AS IDENTITY"),
        "AUTO_INCREMENT must become IDENTITY: {content}"
    );
    assert!(
        content.to_uppercase().contains("BOOLEAN"),
        "tinyint(1) must become BOOLEAN: {content}"
    );
    assert!(
        !content.to_uppercase().contains("ENGINE"),
        "ENGINE= must be stripped: {content}"
    );
    // Exit code reflects the underlying fix outcome (clean or warnings), not failure.
    assert!(
        matches!(output.status.code(), Some(0) | Some(3)),
        "expected exit 0 or 3, got {:?}",
        output.status.code()
    );
}

#[test]
fn dialect_mysql_without_fix_is_usage_error() {
    let dir = tempfile::tempdir().unwrap();
    let input = dir.path().join("dump.sql");
    ...
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(
        stderr.contains("requires --fix"),
        "expected 'requires --fix' error, got: {stderr}"
    );
}
