...
const ERROR_CASES: &[(&str, &str, &str)] = &[
    ...
    ("serial",     "CREATE TABLE t (id SERIAL PRIMARY KEY);",      "SERIAL"),
    ("serial",     "CREATE TABLE t (id SERIAL4 PRIMARY KEY);",     "SERIAL"),
    ("serial",     "CREATE TABLE t (id BIGSERIAL PRIMARY KEY);",   "BIGSERIAL"),
    ("serial",     "CREATE TABLE t (id SERIAL8 PRIMARY KEY);",     "BIGSERIAL"),
    ("serial",     "CREATE TABLE t (id SMALLSERIAL PRIMARY KEY);", "SMALLSERIAL"),
    ("serial",     "CREATE TABLE t (id SERIAL2 PRIMARY KEY);",     "SMALLSERIAL"),
    // JSON / JSONB
    ...
    ("fk",         "CREATE TABLE t (id INT, cid INT REFERENCES c(id));",                    "FOREIGN KEY"),
    ("fk",         "CREATE TABLE t (id INT, cid INT, FOREIGN KEY (cid) REFERENCES c(id));", "FOREIGN KEY"),
    // Temp tables
    ...
    // Partition
    ("partition",  "CREATE TABLE t (id INT, d DATE) PARTITION BY RANGE (d);", "PARTITION"),
    // Truncate
    ("truncate",   "TRUNCATE TABLE orders;", "TRUNCATE"),
    // Trigger
    ("trigger",    "CREATE TRIGGER trg AFTER INSERT ON t FOR EACH ROW EXECUTE FUNCTION f();", "TRIGGER"),
    // Extension
...
const SUGGESTED_COLUMN_TYPES: &[&str] = &[
    "BIGINT GENERATED BY DEFAULT AS IDENTITY (CACHE 1)",
];
...
const FALSE_POSITIVE_CASES: &[(&str, &str)] = &[
    ...
    ("CREATE TABLE t (id UUID PRIMARY KEY, serial_number VARCHAR(100));", "SERIAL"),
    // TEXT column named 'json_data' should not trigger JSON rule
    ("CREATE TABLE t (id UUID PRIMARY KEY, json_data TEXT);", "JSON"),
    // Table named 'temporary_cache' should not trigger TEMP rule
    ("CREATE TABLE temporary_cache (id UUID PRIMARY KEY);", "TEMPORARY"),
    ...
    ("CREATE TABLE t (id UUID PRIMARY KEY, partition_key VARCHAR(50));", "PARTITION"),
    ...
    ("CREATE TABLE t (id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY);", "SERIAL"),
    // GENERATED BY DEFAULT AS IDENTITY should not trigger SERIAL
    ("CREATE TABLE t (id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY);", "SERIAL"),
];
...
const WARNING_CASES: &[(&str, &str)] = &[
    ("ALTER TABLE t ADD COLUMN status VARCHAR(50) DEFAULT 'pending';", "ADD COLUMN"),
    ("ALTER TABLE t ADD COLUMN status VARCHAR(50) NOT NULL;",         "ADD COLUMN"),
];
...
fn warning_detection_matrix() {
    for (sql, expected) in WARNING_CASES {
        let diags = lint_sql(sql);
        assert!(
            diags.iter().any(|d| d.severity == Severity::Warning && d.message.contains(expected)),
...
fn warning_false_positive_matrix() {
    for (sql, unexpected) in WARNING_FALSE_POSITIVES {
        let diags = lint_sql(sql);
        assert!(
            !diags.iter().any(|d| d.severity == Severity::Warning && d.message.contains(unexpected)),
...
fn fixture_sample_migration() {
    ...
    let expected = &[
        "SERIAL", "FOREIGN KEY", "JSON", "TRUNCATE", "TEMPORARY", "array", "ASYNC",
    ];
    for msg in expected {
        assert!(
