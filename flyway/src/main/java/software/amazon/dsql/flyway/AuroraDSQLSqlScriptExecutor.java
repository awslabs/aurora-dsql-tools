 ...
 */
class AuroraDSQLSqlScriptExecutor implements SqlScriptExecutor {

    private static final Logger LOG = Logger.getLogger(AuroraDSQLSqlScriptExecutor.class.getName());
    ...
    AuroraDSQLSqlScriptExecutor(SqlScriptExecutor delegate, Connection connection) {
    ...
    private void waitForJob(String jobId) {
        ...
        try {
            ...
            // pending transaction) before the CALL, mirroring AuroraDSQLSchema.doClean(). Switching to
            ...
            // If this commit raises OCC, the DDL did not commit and AuroraDSQLExecutionTemplate replaying
            ...
            try (CallableStatement cs = connection.prepareCall("CALL sys.wait_for_job(?)")) {
                ...
                LOG.fine("Waited for Aurora DSQL async index build, job_id=" + jobId);
            }
